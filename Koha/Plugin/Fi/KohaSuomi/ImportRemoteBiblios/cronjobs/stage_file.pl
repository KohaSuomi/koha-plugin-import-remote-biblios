...
sub process_batch {
    ...
    print "Add 035a field if not present\n";
    ...
    while (my $rowref = $sth->fetchrow_hashref) {
        my $rec_num = $rowref->{'import_record_id'};
        my $marc_record = MARC::Record->new_from_usmarc($rowref->{'marc'});
        if($marc_record->subfield('035', 'a')) {
            next;
        } else {
            my $field = MARC::Field->new('035', '', '', 'a' => '('.$marc_record->field('003')->data().')'.$marc_record->field('001')->data());
            $marc_record->insert_fields_ordered($field);
            my $marc = $marc_record->as_usmarc();
            my $marcxml = $marc_record->as_xml_record();
            my $sth2 = $dbh->prepare("UPDATE import_records SET marc = ?, marcxml = ? WHERE import_record_id = ?");
            $sth2->execute($marc, $marcxml, $rowref->{'import_record_id'});
            $sth2->finish();
            $num_035a_added++;
        }
    }
...
Number of 035a fields added: $num_035a_added
_SUMMARY_
    if( $params->{match} ) {
